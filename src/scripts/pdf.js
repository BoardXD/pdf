...
const COLORS = {
  subTitle: rgb(0.704, 0.764, 0.864),
  headers: rgb(0.272, 0.396, 0.548),
};

export async function createPdf({ jsonData, weeks }) {
  ...
  await setMonth({ font: customFontBold, month: "Dezembro", pdf });

  const { attendant, plataformAttendant, audioAndVideo, microphones } = await filterByRoles(jsonData);

  await Promise.all([
    addAssign({ assignments: attendant, customFontRegular, customFontBold, headerY: 655, pdf, weeks }),
    addAssign({ assignments: plataformAttendant, customFontRegular, customFontBold, headerY: 485, pdf, weeks }),
    addAssign({ assignments: audioAndVideo, customFontRegular, customFontBold, headerY: 315, pdf, weeks }),
    addAssign({ assignments: microphones, customFontRegular, customFontBold, headerY: 145, pdf, weeks }),
  ]);
...
async function addAssign({ pdf, weeks, assignments, customFontRegular, customFontBold, headerY }) {
